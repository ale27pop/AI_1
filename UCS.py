...
pygame.display.set_caption("Uniform Cost Search Path Finding Algorithm")
...
PASTEL_OPEN = (100, 255, 100)  # Open nodes
BRIGHT_PINK = (255, 105, 180)  # Path
DELAY_TIME = 0.05  # Delay time for coloring
...
def reconstruct_path(came_from, current, draw):
    ...
    while current in came_from:
        ...
        current.make_path()
        draw()
        time.sleep(DELAY_TIME)  # Slow down the coloring
    
    # Reverse the path to display from start to end
    path.reverse()
    print_path(path)
...
def algorithm(draw, grid, start, end):
    ...
    open_set.put((0, count, start))  # (priority, count, spot)
    came_from = {}
    ...
    while not open_set.empty():
        ...
        for neighbor in current.neighbors:
            temp_g_score = g_score[current] + 1  # Cost to move to neighbor is always 1

            if temp_g_score < g_score[neighbor]:  # consider new path if better
                ...
                if neighbor not in open_set_hash:
                    ...
                    neighbor.make_open()  #neighbor open
                    draw()
        ...
        if current != start:
            current.make_closed()  #current node closed
            draw()
            time.sleep(DELAY_TIME) 
