...
def reconstruct_path(came_from, current, draw):
    ...
    # reverse it to display from start to end
    path.reverse()
    print_path(path)
...
def algorithm(draw, grid, start, end):
    ...
    while not open_set.empty():
        ...
        for neighbor in current.neighbors:
            temp_g_score = g_score[current] + 1  # Assuming cost to move to neighbor is always 1

            if temp_g_score < g_score[neighbor]:  #new path if better
                ...
                if neighbor not in open_set_hash:
                    ...
                    draw()
                    time.sleep(DELAY_TIME)  # slow down
